<div>
    <flux:field>
        ...
        <flux:text class="mb-4 text-sm">Faça o upload de uma ou mais imagens. A primeira imagem será definida como capa
            automaticamente</flux:text>
        ...
        @if (!empty($images))
            <div class="mb-4">
                ...
                @if ($model)
                    <div class="mt-4">
                        <flux:button wire:click="saveImages" variant="primary" size="sm" icon="arrow-up-tray">
                            Salvar Imagens
                        </flux:button>
                    </div>
                @endif
            ...
            <div>
                ...
                <div class="grid grid-cols-2 gap-4 md:grid-cols-4">
                    @foreach ($existingImages as $existingImage)
                        <div class="relative group">
                            <img src="{{ $existingImage['url'] }}" alt="Image"
                            ...
                            <div
                                ...
                                @if (!$existingImage['is_cover'])
                                    <button type="button" wire:click="setCover({{ $existingImage['id'] }})"
                                        class="p-1 bg-blue-600 text-white rounded-full" title="Definir como capa">
                                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                                d="M5 13l4 4L19 7" />
                                        </svg>
                                ...
                                <button type="button" wire:click="removeExistingImage({{ $existingImage['id'] }})"
                                    ...
                                    class="p-1 bg-red-600 text-white rounded-full" title="Remover imagem">
                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                            d="M6 18L18 6M6 6l12 12" />
                                    </svg>
